...
class MqttConnection implements MqttCallback {
		...
		try {
			...
			IMqttActionListener listener = new MqttConnectionListener(
				...
				public void onSuccess(IMqttToken asyncActionToken) {
                    ...
                    Logger.t(TAG).d(">>>> 리얼실행 connect success");
				...
				public void onFailure(IMqttToken asyncActionToken,
                    ...
                    service.traceDebug(TAG, "connect fail, call connect to reconnect.reason:" + exception.getMessage());  //youngil  변경

                    Logger.t(TAG).d(">>>> 리얼실행 connect fail, call connect to reconnect.reason:" + exception.getMessage());
			...
			if (myClient != null) {
				if (isConnecting ) {
					...
					setConnectingState(true); //yuungil del
			...
			// if myClient is null, then create a new connection
				...
				service.traceDebug(TAG,"Do Real connect!");
				setConnectingState(true); //youngil del
				myClient.connect(connectOptions, invocationContext, listener);
			}
		} catch (Exception e) {
	...
	private void doAfterConnectSuccess(final Bundle resultBundle) {
		...
		deliverBacklog();
		//setConnectingState(false);  //youngil 주석
        setConnectingState(true); //youngil 변경
		disconnected = false;
		releaseWakeLock();
	}
	...
	public void connectionLost(Throwable why) {
		...
		disconnected = true;
        setConnectingState(false); //youngil add
		try {
	...
	private class MqttConnectionListener implements IMqttActionListener {
		...
		@Override
		public void onSuccess(IMqttToken asyncActionToken) {
            Logger.t(TAG).d(">>> 여기 실행 onSuccess()");
			service.callbackToActivity(clientHandle, Status.OK, resultBundle);
		}

		@Override
		public void onFailure(IMqttToken asyncActionToken, Throwable exception) {
            Logger.t(TAG).d(">>> 여기 실행 onFailure()");
			resultBundle.putString(MqttServiceConstants.CALLBACK_ERROR_MESSAGE,
					exception.getLocalizedMessage());
	...
	*/
	synchronized void reconnect() {
        ...
        Logger.t(TAG).d("reconnect(), is isConnecting=%s, service.isOnline()=%s",String.valueOf(isConnecting),String.valueOf(service.isOnline()));
		if (isConnecting) {
		...
		if (disconnected && !cleanSession) {
            ...
            Logger.t(TAG).d(">>>> Do Real Reconnect!");
			...
			try {
				...
				setConnectingState(true); //youngil 제거
			} catch (MqttException e) {
