...
public class MqttService extends Service implements MqttTraceHandler {
  ...
  private BroadcastReceiver m_reconnectAlarmReceiver;
  private volatile boolean m_reconnectStarted = false;
  ...
  public String getClient(String serverURI, String clientId, String contextId,MqttClientPersistence persistence) {
    String clientHandle = serverURI + ":" + clientId+":"+contextId;
     ...
     Logger.t(TAG).d("clientHandle = %s",clientHandle);
  ...
  public void onCreate() {
	...
	public void onDestroy() {
        Logger.t(TAG).d("**** service destory");
...
//    Logger.t(TAG).d(message);
  ...
  private void registerBroadcastReceivers() {
        ...
        Logger.t(TAG).d(">>>> registerBroadcastReceivers() , networkConnectionMonitor=%s",String.valueOf(networkConnectionMonitor));
  ...
  private void unregisterBroadcastReceivers(){
    ...
    Logger.t(TAG).d("**** unregisterBroadcastReceivers() , networkConnectionMonitor=%s",String.valueOf(networkConnectionMonitor));
  	if(networkConnectionMonitor != null){
  ...
  private class NetworkConnectionIntentReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
            ...
            Logger.t(TAG).d("**** NetworkConnectionIntentReceiver() Internal network status receive");
    ...
    //재접속 알람 시작
    public void reconnectAlarmStart(String clientId){



        Logger.t(TAG).d("**** alarmContent called , clientId=%s",clientId);
        String action ="MqttService.reconnectAlarm."+clientId;

        //브로드캐스트 리시버를 인텐트 필터에 등록한다.
        m_reconnectAlarmReceiver = new ReconnectAlarmReceiver();
        registerReceiver(m_reconnectAlarmReceiver, new IntentFilter(action));

        m_reconnectPendingIntent = PendingIntent.getBroadcast(MqttService.this, 0, new Intent(
                action), PendingIntent.FLAG_UPDATE_CURRENT);

        reconnectAlarmSchedule(10000);

        m_reconnectStarted=true;
    }

    //재점속 알람 종료
    public void reconnectAlarmStop() {

        AlarmManager alarmManager = (AlarmManager)getSystemService(Service.ALARM_SERVICE);
        alarmManager.cancel(m_reconnectPendingIntent);

        Logger.t(TAG).d("**** Unregister alarmreceiver to MqttService");

        if(m_reconnectStarted){
            m_reconnectStarted = false;
            try{
                unregisterReceiver(m_reconnectAlarmReceiver);
    ...
    private class ReconnectAlarmReceiver extends BroadcastReceiver  {
        ...
        private String wakeLockTag = "MqttService.reconnectAlarm.";

        @Override
        public void onReceive(Context context, Intent intent) {
            Logger.t(TAG).d("**** Called Alarm Receiver ");

            int count = intent.getIntExtra(Intent.EXTRA_ALARM_COUNT, -1);
            Logger.t(TAG).d("***** alarm receiver " + count + " times.");
    ...
    public void reconnectAlarmSchedule(long delayInMilliseconds) {

        // 5초후에 알람시작
        long startTime = SystemClock.elapsedRealtime() + 5*1000;

        // 알랑이 반복되는 주기 (10초)
        long cycleTime = 10*1000;

        long nextAlarmInMilliseconds = System.currentTimeMillis()
                + delayInMilliseconds;

        Logger.t(TAG).d(" Schedule next reconnect alarm at " + nextAlarmInMilliseconds);

        AlarmManager alarmManager = (AlarmManager)getSystemService(Service.ALARM_SERVICE);
        //alarmManager.setRepeating(AlarmManager.RTC_WAKEUP, startTime, cycleTime, m_reconnectPendingIntent);
        alarmManager.setRepeating(AlarmManager.ELAPSED_REALTIME_WAKEUP, startTime, cycleTime, m_reconnectPendingIntent);
        //alarmManager.set(AlarmManager.RTC_WAKEUP, nextAlarmInMilliseconds,m_reconnectPendingIntent);
    }
