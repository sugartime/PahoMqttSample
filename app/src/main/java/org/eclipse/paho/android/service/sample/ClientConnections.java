...
public class ClientConnections extends ListActivity {
  ...
  private void connectAction(Bundle data) {
    ...
    String ssl_key = (String) data.get(ActivityConstants.ssl_key); //mnt/sdcard/pem/m2mqtt_srv.bks
    String uri = null;
    if (ssl) {
    ...
    uri = uri + server + ":" + port; //ssl://172.22.105.163:8883

    MqttAndroidClient client;
    client = Connections.getInstance(this).createClient(this, uri, clientId);

    Logger.t("ClientConnections").d("ssl=%s",String.valueOf(ssl));
    ...
    if (ssl){
        try {
        	if(ssl_key != null && !ssl_key.equalsIgnoreCase(""))
        	{
        		...
        		conOpt.setSocketFactory(client.getSSLSocketFactory(key,"ggpol123"));
        	}
