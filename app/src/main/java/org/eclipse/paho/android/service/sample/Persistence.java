...
public class Persistence extends SQLiteOpenHelper implements BaseColumns {
  ...
  private Context context = null; //youngil add
    ...
    for (int i = 0; i < c.getCount(); i++) {
      ...
      if (ssl){
         try {
                if(ssl_key == null)
                {
                    //FileInputStream key = new FileInputStream(R.raw.raw_key_file);
                    InputStream key = null;
                    key = context.getResources().openRawResource(R.raw.m2mqtt_srv);
                    opts.setSocketFactory(connection.getClient().getSSLSocketFactory(key,"ggpol123"));
                }
